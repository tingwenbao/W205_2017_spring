...
8. To have Postgres connect to Tableau: First, make sure port 5432 is open for your instance. Then, set up your ODBC like ODBC_setup.PNG in the directory (with password : pass as defined in the code).Then open Tableau, in connect choose PostgresSQL and put in the same information as you did for ODBC and sign in.
9. To create the top 20 word bar chart in Tableau: pull Word and Count onto columns and rows, then chose bar chart in "Show me". Move your mouse onto the top of the chart and click the small bar chart icon to sort it desc. Then right click in the column name section, choose filter->top->set top 20 by field. The result graph Plot.PNG could be found under current directory
